...
def enumerate_version_dirs(source_path: Path) -> Iterator[Path]:
...
def generate_tasks_for_format(
    ...
    *,
    ...
    # Create tasks for real files first. These also create per-version grouping tasks
    ...
    # Generate cloning tasks for versions that are identical to others. These depend on
    # the file generation tasks we created in the previous loop.
    for parent_version, children in rules.identical_versions.items():
        if str(parent_version) not in built_versions:
            sys.exit(
                f"Can't clone {len(children)} child versions from '{parent_version}'"
                " because that version doesn't exist.\nExisting versions: "
                + ", ".join(map(str, sorted(built_versions)))
                + "\nFailed child versions: "
