...
def create_yaml_postprocess_tasks(  # noqa: PLR0913
    ...
    *,
    ...
    """Create tasks to postprocess a YAML file to JSON, then back to clean YAML.

    Many YAML files contain constructors like `!enrollment-source-type-code` to factor
    out recurring field types, like large enums. These cannot be rendered by users that
    don't have those constructors defined. The tasks generated by this function expand
    all constructors so that no special handling is required.

    Arguments:
        yaml_source (pathlib.Path):
            A valid YAML file that needs to be processed.
        target_dir (pathlib.Path):
            The directory to write the rendered files into.
        extra_file_deps ([pathlib.Path]):
            Optional. An iterable of any additional files that the yaml source is
            dependent on, such as the auxiliary files defining the constructors that the
            unprocessed YAML file may require.
        output_stem (str):
            Optional. The base name for the files that will be written. If not given,
            it will be the same as `yaml_source`. The `.json` and `.yaml` extensions
            are appended to this filename stem.
        version (str):
            The PCUG version that this file is for.
        record_type (str):
            The type of the record that the file describes, such as "header", "trailer",
            etc., not the file type.
    """
    stem = output_stem or yaml_source.stem
    json_path = target_dir / (stem + ".json")
    yaml_path = target_dir / (stem + ".yaml")
