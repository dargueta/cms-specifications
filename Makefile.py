...
def generate_tasks_for_identical_versions(
    format_name: str,
    version: VersionSpec,
    children: list[VersionSpec],
    build_rules_file: Path,
    ...
    child_output_dirs = [format_build_root / str(v) for v in children]
    version_output_dir = format_build_root / str(version)

    try:
        task.register(
            functools.partial(
                _generic_link_or_copy_task,
                parent=version_output_dir,
                children=child_output_dirs,
            ),
            name=f"{format_name}_{version.major}_{version.minor}",
            inputs=[version_output_dir],
            outputs=child_output_dirs,
        )
    except Exception as err:
        err.add_note(
            "Something is broken with the [identical-versions] section of the file"
            f" format {format_name!r} build configuration at {format_source_root}\n"
            f"Broken version: {version}\n"
            f"Declared children: " + ", ".join(map(str, children)) + "\n"
    ...
    already_handled_versions |= dict.fromkeys(children, value=version)
